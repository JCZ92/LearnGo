...
func (s *Server) Handler(conn net.Conn) {
	...
	go func() {
		buf := make([]byte, bufferSizeForMessageFromClient) // creat a buffer to receive messages from client
		for {
			n, err := conn.Read(buf)
			if n == 0 {
				user.Offline()
	...
	for {
		select { 
			...
			// in this case, every select will wait at most [timeoutLogOffInSec] seconds, 
			// if in this [timeoutLogOffInSec] seconds, an isActive is received, will reset the timer
			// if timeout, the user is forced to log off
		case <-isActive:
			// do nothing, user is active
		case <-time.After(time.Second * time.Duration(timeoutLogOffInSec)):
